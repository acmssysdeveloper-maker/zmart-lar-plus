(() => {
  ...
  async function pushSalesToSupabase(){
    ...
    for(const sale of state.sales){
      await sbFetch('/rest/v1/sales?on_conflict=id',{method:'POST',prefer:'resolution=merge-duplicates,return=minimal',
        body:JSON.stringify([{id:sale.id,property:sale.property,schedule:sale.schedule,audit:sale.audit,settings:sale.settings,updated_at:new Date().toISOString()}])});
  ...
  function seedSale(title='Venda do imóvel', total=350000, entryTotal=50000, parcelCount=60, entryCount=5, firstDueDate=today()){
    ...
    return {id:uid(),property:{title,total,entryTotal,description:'Acompanhamento financeiro da compra e venda'},installments:makeInstallments(entryTotal,entryCount,firstDueDate,{...schedule,totalParcelValue:parcelTotal}),receiptInbox:[],audit:[],settings:{validationTolerance:0.01,maxFileMB:20,ocrLanguage:'por'},schedule,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  }
  ...
  function syncActiveSale(){const sale=state.sales?.find(x=>x.id===state.activeSaleId);if(!sale)return;sale.property=state.property;sale.installments=state.installments;sale.receiptInbox=state.receiptInbox;sale.audit=state.audit;sale.settings=state.settings;sale.schedule=state.schedule||sale.schedule;sale.updatedAt=new Date().toISOString();}
  ...
  function dashboardPage(){
    ...
    <section class="section-head"><div><h2>Próximo passo</h2><p>${next?'O próximo compromisso financeiro.':'Você está em dia.'}</p></div>${next?linkAction('openNext','ver parcela'):''}</section>${next?`<article class="next-card"><div><span>${esc(next.label)}</span><strong>${money(next.value)}</strong><small>Vencimento ${dateBR(next.dueDate)} · ${next.status==='paid'?'Pago':next.status==='partial'?'Parcial':'Pendente'}</small></div><div class="next-actions">${role==='vendedor'&&next.status!=='paid'?linkAction('validateNext','validar','green-link'):''}${linkAction('nextReceipt','comprovante')}</div></article>`:`<div class="empty-card">Tudo certo por aqui. Continue acompanhando sua evolução.</div>`}`;
  }
  ...
  function bindPage(){
    ...
    $('#backupExport')?.addEventListener('click',exportBackup); $('#backupExport2')?.addEventListener('click',exportBackup); $('#backupFile')?.addEventListener('change',e=>importBackup(e.target.files[0])); $('#saveGoogleClient')?.addEventListener('click',()=>{const v=$('#googleClientId').value.trim();if(v)localStorage.setItem('zmart_google_client_id',v);showToast(v?'Client ID salvo.':'Informe o Client ID.');}); $('#connectDrive')?.addEventListener('click',connectGoogleDrive); $('#openCalendar')?.addEventListener('click',openCalendar); $('#paymentReport')?.addEventListener('click',()=>makePDF('paid')); $('#openNext')?.addEventListener('click',()=>{currentPage='parcelas';renderPage('parcelas');}); $('#validateNext')?.addEventListener('click',()=>{const n=state.installments.find(i=>i.status!=='paid'); if(n)paymentModal(n.id);}); $('#nextReceipt')?.addEventListener('click',()=>{const n=state.installments.find(i=>i.status!=='paid'); if(n)receiptModal(n.id);});
  }
  ...
  function paymentModal(id){
    ...
    $('#payForm').addEventListener('submit',async e=>{
      ...
      try{
        if(file){const parsed=await parseReceiptFile(file);const paymentKey=i.id+'_p_'+uid();const receiptRef=await saveOriginalReceiptFor(file,i.id,paymentKey,parsed);i.receipt={...(i.receipt||{}),validation:{status:'review',source:'payment-form'}};closeModal();reviewAndValidateModal(i.id,parsed,value,date,$('#payNote').value.trim(),receiptRef);}
  ...
  function historiesByMonth(){
    ...
    ordered.forEach(i=>{
      ...
      if(i.status==='paid'){
        const v=Number(i.paidValue||i.value)||0;
        monthly[ym].total+=v;
